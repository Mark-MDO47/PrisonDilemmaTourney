...
#
# algo_mdo_gradual_var.py - Prisoner's Dilemma tournament algorithm file
#
# This one is a variant of algo_mdo_gradual. Here is the algo_mdo_gradual description from
...
#
# This variant (described in above reference) merely changes the number of times to DEFECT in response to a
#           DEFECT by the opponent:
#    normal - "n" consecutive defects after the nth opponent defect
#    variant - (n-1)*n/2
#
# The algo_mdo_gradual_var algorithm behaves as follows:
...
#                       (n-1)*n/2 - 1 (where "n" is total number opponent defects) and return choices.DEFECT
#       else if the state is choices.DEFECT
#           if "more defects" is > 0, decrement "more defects" and return choices.DEFECT
#           else if "more defects" is == 0, set state choices.COOPERATE, set "more cooperates" to 1,
#                       and return choices.COOPERATE
#
...
def algo_mdo_gradual_var(selfHist, oppHist):
